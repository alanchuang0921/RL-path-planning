...
def directly_go_to_target(quaternion,p_w_new,robot_initial_pos):
    """
    輸入工件的目標位置以及目標角度(以軸角表示法表示),以及手臂的初始姿態，輸出手臂六軸角度
    p_w_new:工件目標位置
    axis:旋轉軸
    angle:旋轉角
    ...
    p_w = np.array([0.81687, 0.448782, 0.0628227])#實際機台的工件位置 
...
class World(Env):
    def __init__(self):
       ...
       #----------------------------------------------------
        ...
        # t_contactpoint_frame = np.array([0.0100017, 0.5567, 0.41])
        t_contactpoint_frame = np.array([0.100017, 0.5607, 0.41])#此處為砂帶上接觸點的世界座標，可由"座標轉換.ipynb"計算其歐拉角(軸-角--->歐拉)
        ...
        self.start_points = self.get_init_point_on_path()
    ...
    def get_state(self):
        ...
        #夾爪
        outer_finger_1_node = self.supervisor.getFromDef("outer_finger_1")
        contact_points_8 = outer_finger_1_node.getContactPoints()
        outer_finger_2_node = self.supervisor.getFromDef("outer_finger_2")
        contact_points_9 = outer_finger_2_node.getContactPoints()
        robotiq_base_node = self.supervisor.getFromDef("robotiq_base")
        contact_points_10 = robotiq_base_node.getContactPoints()
        if len(contact_points_8) > 0 or len(contact_points_9) > 0 or len(contact_points_10) > 0:
            gripper_contact=1
        else:
            gripper_contact=0


        if len(contact_points_1) > 0 or len(contact_points_2) > 0 or len(contact_points_3) > 0 or len(contact_points_4) > 0 or len(contact_points_5) > 0 or len(contact_points_6) > 0 or gripper_contact>0:
    ...
    def reset(self, seed=None, options=None):#重置世界
        ...
        if tracker.get_success_rate()>0.01:
            self.next_stage = 1

        if self.next_stage == 1: 
            while self.supervisor.step(self.timestep) != -1:
                # self.grinder_translation_field.setSFVec3f([0.3,0.95,1.03529e-08])#手臂移動到初始位置後再將砂帶機台拿回來//不小心設成([0.3,0.95,1.03529e-08])
    ...
    def calculate_reward(self,current_state,next_state):
        ...
        #------------------------------------------------------------------------------
        if (position_error_prime<0.02 and orientation_error_prime<10) :
            r6 = 1
            get_to_target=True
        else:
            r6=0
        ...
        r8 = -(0.2*delta_angle_5+0.1*delta_angle_4+0.1*delta_angle_3 + 0.1*delta_angle_2+0.1*delta_angle_1)/100
        ...
        # print("r4=",r4)
        # print("r5=",r5)
        r34_withWieght = a3*r3+a4*r4
        r34_noWieght = r3+r4
        reward = a1*r1+a2*r2+a3*r3+a4*r4+a5*r5+a6*r6 + a8*r8
        rewards=[r1,r2,r3,r4,r5,r6,r7,r8,r34_withWieght,r34_noWieght]
