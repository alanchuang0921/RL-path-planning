...
model_path = "lowlevel_models/20250703-235829_4faceS_wp7cm_changWithSuccessRate_noCrashDone_a4_1.65_a5_0.5_final_4505600_steps"
model_path = "lowlevel_models/20250708-130144_4faceS_wp7cm_changWithSuccessRate_noCrashDone_thres_0.035_4096000_steps"
model_path = "lowlevel_model_test_0718/20250718-213112_4faceS_wp7cm_changWithSuccessRate_noCrashDone_a1_20_3900000_steps"
# model_path = "lowlevel_model_test_0805/20250805-000715_4faceS_wp7cm_changWithSuccessRate_noCrashDone_a1_20_4000000_steps"
...
class path_connect_env(Env):
    ...
    def execute_task(self,samplepoint_num):
        ...
        while not done:
            ...
            obs, reward, done, truncated, info = world.step(action[0])
            ...
            # if "crash" in info or steps>249:
            #     # print("info_crash=",info["crash"])
            ...
            if info["crash"]==True or truncated:
                # print("done = ",done)
                # print("truncated = ",truncated)
                # print("success rate",info["success rate"])
...
class path_select_env(Env):
    ...
    def get_pos_ori_errors(self):
        return world.position_errors,world.orientation_errors
    ...
    def step(self, action):#輸入採樣點的順序，執行該路徑並回傳總路徑長
        ...
        displacement,steps,execute_task_info= self.worker.execute_task(target_point_index) #選擇完目標採樣點後，進行路徑串接
        ...
        if set(self.taget_start_points).issubset(set(self.done_paths)) or execute_task_info["crash"]:
        ...
        if self.done or truncated:
            ...
            info["crash"] = execute_task_info["crash"]
    ...
    def save_robot_posture_as_csv(self,file_path):
        world.save_robot_posture_as_csv(file_path)

    def save_joint_velocity_as_csv(self,file_path):
        world.save_joint_velocity_as_csv(file_path)
